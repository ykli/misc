...
**********************************************************************/
/*
 * $Id: bitstream.h,v 1.3 2006/02/20 22:53:29 kyang Exp $
 */
...
#ifdef WIN32
static unsigned short ByteSwap16(unsigned short x)
{
    uchar_t c0,c1;
    c0 = (x&0xFF);
    c1 = (x>>8);
...
#define bit2byte(a) (((a)+BYTE_NUMBIT-1)>>3)


typedef struct
{
#ifdef OLDPUTBIT
    uchar_t *data;      /* data bits */
    long_t numBit;          /* number of bits in buffer */
    long_t size;            /* buffer size in bytes */
    long_t currentBit;      /* current bit position in bit stream */
    long_t numByte;         /* number of bytes read/written (only file) */
#else
    uint32_t bit_buf;
    int32_t bit_left;
...
#ifdef OLDPUTBIT
int32_t PutBit(BitStream *bitStream,
               ulong_t data,
               int32_t numBit);

int32_t PutBit2(BitStream *bitStream,
               ulong_t data,
               int32_t numBit);
#else
