...
***********/
/*
 * $Id: huffman.c,v 1.7 2006/07/13 00:49:49 kyang Exp $
 */
...
static int32_t faac_huff_data[2][5*FRAME_LEN];
static int32_t faac_huff_len[2][5*FRAME_LEN];
#endif
...
#ifdef FAAC_STATIC_MEMORY        
        coderInfo[channel].data = (int32_t*)faac_huff_data[channel];
        coderInfo[channel].len = (int32_t*)faac_huff_len[channel];
#else
        coderInfo[channel].data = (int32_t*)AllocMemory((5*FRAME_LEN)*sizeof(int32_t));
        coderInfo[channel].len = (int32_t*)AllocMemory((5*FRAME_LEN)*sizeof(int32_t));
#endif
    }
}
...
int32_t BitSearch(CoderInfo *coderInfo,
                  int32_t *quant)  /* Quantized spectral values */
/*
...

{
    ...
    int32_t bit_stats[240][3];
    int32_t total_bit_count;
    int32_t levels;
    int32_t pow2levels;
    int32_t fracpow2lev;

    /* Set local pointer to coderInfo book_vector */
    int32_t* book_vector = coderInfo -> book_vector;
	...
	fracpow2lev = pow2levels + (coderInfo->nr_of_sfb >> i);
        ...
        for (j=pow2levels; j < fracpow2lev; j++) {
            ...
            if (i>0){  /* not on the lowest level, grouping more than one signle scalefactor band per section*/
                if  (bit_stats[j][0] < bit_stats[j<<1][0] + bit_stats[(j<<1)+1][0]){
                    ...
                    bit_stats[j][0] = bit_stats[j<<1][0] + bit_stats[(j<<1)+1][0];
                }
	    } 
            else
#endif
                          ...
                          int32_t min_book_choice[112][3])
{
    ...
    */
    ...
    int32_t max_sb_coeff;
    int32_t book_choice[12][2];
    int32_t total_bits_cost = 0;
    int32_t offset, length, end;
    int32_t q;

    /* set local pointer to sfb_offset */
    int32_t *sfb_offset = coderInfo->sfb_offset;
    int32_t nr_of_sfb = coderInfo->nr_of_sfb;
...

{
    ...
    N = -1;
    y = faac_fabs(REAL_ICONST(input));
    x = y>>4;

    while (x >= REAL_ICONST(1)) {
        N++;
        x = x>>1;
    }

    *len_esc_sequence = (N<<1) + 5;  /* the length of the escape sequence in bits */

/*    output = (int)((pow(2,N) - 1)*pow(2,N+5) + y - pow(2,N+4)); */
    output = ((1<<N) - 1)*(1<<(N+5)) + (int32_t)REAL2INT(y) - (1<<(N+4));
               ...
               int32_t length)
{
   ...
   */
    ...
    int32_t* data = coderInfo->data;
    int32_t* len  = coderInfo->len;
#ifdef DRM
    int32_t* num_data = coderInfo->num_data_cw;
    int32_t cur_cw_len;
    int32_t max_esc_sequ = 0;
#endif
    ...
    case 1:
        for (i=offset;i<offset+length;i=i+4) {
            index = MUL27(quant[i]) + MUL9(quant[i+1]) + MUL3(quant[i+2]) + quant[i+3] + 40;
    ...
    case 3:
        for(i=offset;i<offset+length;i=i+4){
            index = ABS(MUL27(quant[i])) + ABS(MUL9(quant[i+1])) + ABS(MUL3(quant[i+2])) + ABS(quant[i+3]);
    ...
    case 5:
        for(i=offset;i<offset+length;i=i+2){
            index = MUL9(quant[i]) + (quant[i+1]) + 40;
    ...
    case 7:
        for(i=offset;i<offset+length;i=i+2){
            index = ABS(MUL8(quant[i])) + ABS(quant[i+1]);
    ...
    case 9:
        for(i=offset;i<offset+length;i=i+2){
            index = ABS(MUL13(quant[i])) + ABS(quant[i+1]);
                        ...
                        int32_t writeFlag)
{
  /*
    ...
    */
    ...
    int32_t bit_count = 0;
    int32_t previous;
    int32_t max, bit_len/*,sfbs*/;
    int32_t max_sfb,g,band;
    int32_t sect_cb_bits = 4;
...

{
    ...
    register int32_t i,j;
    int32_t bit_count=0;
    int32_t diff,length,codeword;
    int32_t previous_scale_factor;
    int32_t previous_is_factor;       /* Intensity stereo */
    int32_t index = 0;
    int32_t nr_of_sfb_per_group;

    /* set local pointer to coderInfo elements */
    int32_t* scale_factors = coderInfo->scale_factor;
