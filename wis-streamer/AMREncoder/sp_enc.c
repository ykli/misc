...
/* 
 * debug routine: pre_process->lpc->lsp->weight->prebig->ol_ltp->subframePreProc->cl_ltp
 *				  cbsearch->gainQuant->subframePostProc
 */
#ifdef AMR_DEBUG
//#define AMR_DEBUG_PREPROCESS	1
//#define AMR_DEBUG_LPC	1
//#define AMR_DEBUG_LSP	1
//#define AMR_DEBUG_WEIGHT	1
//#define AMR_DEBUG_PREBIG	1
//#define AMR_DEBUG_OL_LTP	1
//#define AMR_DEBUG_SUBFRAMEPREPROC	1
//#define AMR_DEBUG_CL_LTP	1
//#define AMR_DEBUG_CBSEARCH	1
//#define AMR_DEBUG_GAINQUANT	1
//#define AMR_DEBUG_SUBFRAMEPOSTPROC	1
#endif

#ifdef AMR_DEBUG
void amr_dump_real(char *message, real_t *sample, int nsamples)
{
    int i;

    printf("dumping %s start--------------------------\n",message);
    printf("sample size = %d\n",nsamples);
    for (i = 0; i < nsamples; i++) {
        printf("sample[%d] = %.8f\n",i,REAL2FLOAT(sample[i]));
    }
    printf("dumping %s end--------------------------\n",message);
}

void amr_dump_real32(char *message, real_32_t *sample, int nsamples)
{
    int i;
...
void amr_dump_frac(char *message, frac_t *sample, int nsamples)
{
    int i;
    ...
    printf("sample size = %d\n",nsamples);
    for (i = 0; i < nsamples; i++) {
        printf("sample[%d] = %.8f\n",i,FRAC2FLOAT(sample[i]));
    }
    printf("dumping %s end--------------------------\n",message);
}

void amr_dump_int64(char *message, int64_t *sample, int nsamples)
{
    int i;
...
void amr_dump_int32(char *message, int32_t *sample, int nsamples)
{
    int i;
...
void amr_dump_int16(char *message, int16_t *sample, int nsamples)
{
    int i;
...
void amr_dump_float(char *message, float *sample, int nsamples)
{
    int i;
...
void amr_float2real(float *src, real_t *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = REAL_CONST(src[i]);
}

void amr_float2real32(float *src, real_t *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = (real_t)REAL_CONST(src[i]);
}

void amr_real2float(real_t *src, float *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = REAL2FLOAT(src[i]);
}

void amr_real322float(real_t *src, float *dest, int nsamples)
{
    int i;
...
void amr_float2frac(float *src, frac_t *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = FRAC_CONST(src[i]);
}

void amr_float2frac32(float *src, frac_32_t *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = (frac_32_t)FRAC_CONST(src[i]);
}

void amr_float2int64(float *src, int64_t *dest, int nsamples)
{
    int i;
...
void amr_int642float(int64_t *src, float *dest, int nsamples)
{
    int i;
...
void amr_frac2float(frac_t *src, float *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = FRAC2FLOAT(src[i]);
}

void amr_real2real32(real_t *src, real_32_t *dest, int nsamples)
{
    int i;

    for (i=0;i<nsamples;i++)
        dest[i] = (real_32_t)src[i];
}

void amr_real322real(real_32_t *src, real_t *dest, int nsamples)
{
    int i;
...
typedef struct
{
   real_32_t lsp_hist[M * DTX_HIST_SIZE];
   real_t log_en_hist[DTX_HIST_SIZE];
...
typedef struct
{
   real_t prev_alpha;   /* previous adaptor output, */
   real_t prev_gc;   /* previous code gain, */
   real_t ltpg_mem[LTPG_MEM_SIZE];   /* LTP coding gain history, */
...
typedef struct
{
   real_t past_rq[M];   /* Past quantized prediction error */
}Q_plsfState;
...
typedef struct
{
   real_32_t old_A[M + 1];   /* Last A(z) for case of unstable filter */
}LevinsonState;
...
{
   /* Speech vector */
   real_t old_speech[L_TOTAL];
   real_t *speech, *p_window, *p_window_12k2;
   real_t *new_speech;   /* Global variable */


   /* Weight speech vector */
   real_32_t old_wsp[L_FRAME + PIT_MAX];
   real_32_t *wsp;


   /* OL LTP states */
   Word32 old_lags[5];
   real_t ol_gain_flg[2];


   /* Excitation vector */
   real_t old_exc[L_FRAME + PIT_MAX + L_INTERPOL];
   real_t *exc;

   /* Zero vector */
   real_t ai_zero[L_SUBFR + MP1];
   real_t *zero;

   /* Impulse response vector */
   real_t *h1;
   real_t hvec[L_SUBFR * 2];
   ...
   /* Filter's memory */
   real_t mem_syn[M], mem_w0[M];
   real_32_t mem_w[M];
   real_t mem_err[M + L_SUBFR], *error;
   real_t sharp;
}cod_amrState;
...
INLINE static int64_t Dotproduct40_fixed( real_t *x, real_t *y )
{
    int64_t acc;
    register int i;
#ifdef FIXED_USE_ASM
...
#ifdef FIXED_USE_ASM
    FIXED_MUL(hi,lo,REAL2INT32(x[0]),REAL2INT32(y[0]));
    for ( i=1; i<40; i++ )
       FIXED_MADD(hi,lo,REAL2INT32(x[i]),REAL2INT32(y[i]));
    acc = FIXED_INT64(hi,lo);
#else
   acc = 0;
   for ( i=0; i<40; i++ )
       acc += REAL2INT32(x[i])*REAL2INT32(y[i]);
#endif

   return( acc );
}

INLINE static real_t Dotproduct40_fix2( real_t *x, real_t *y )
{
    real_t acc;
    register int i;
#ifdef FIXED_USE_ASM
...
#ifdef FIXED_USE_ASM
    FIXED_MUL(hi,lo,REAL32(x[0]),REAL32(y[0]));
    for ( i=1; i<40; i++ )
       FIXED_MADD(hi,lo,REAL32(x[i]),REAL32(y[i]));
    acc = FIXED_INT64_R(hi,lo);
#else
   acc = REAL_ICONST(0);
   for ( i=0; i<40; i++ ) {
       acc += MUL_R(REAL32(x[i]),REAL32(y[i]));
   }
#endif
 ...
 *    void
 */
static void Autocorr( real_t x[], int64_t r[], const real_t wind[] )
{
   Word32 i, j;   /* Counters */
   real_t y[L_WINDOW + M + 1];   /* Windowed signal */
   int64_t sum;   /* temp */
   ...
   for ( i = 0; i < L_WINDOW; i++ )
      y[i] = MUL_R(REAL32(x[i]),REAL32(wind[i]));
 
   /*
    * Zero remaining memory
    */
   memset( &y[L_WINDOW], 0, (M+1)*sizeof(real_t) );
   ...
   for ( i = 0; i <= M; i++ ) {
      ...
      for ( j = 0; j < L_WINDOW; j += 40 ) {
         sum += Dotproduct40_fixed( &y[j], &y[j + i] );
      }
      r[i] = sum;
   }
}
 ...
 *
 */
static void Levinson( real_32_t *old_A, int64_t *r, real_32_t *A, real_32_t *rc )
{
   int64_t sum, err;
   real_32_t at;
   Word32 l, j, i;
   real_32_t rct[M];   /* temporary reflection coefficients  0,...,m-1 */
 
   rct[0] = -(REAL_ICONST(r[1])/r[0]);
   A[0] = REAL_ICONST(1);   // Ken: Is it necessary to set A's type to frac_t?
   A[1] = rct[0];
   err = r[0] +  REAL2INT(r[1]*rct[0]);
   ...
   for ( i = 2; i <= M; i++ ) {
      ...
      for ( j = 0; j < i; j++ )
         sum += REAL2INT(r[i - j]*A[j]);
      rct[i - 1] = -REAL_ICONST(sum)/err;

      for ( j = 1; j <= ( i>>1 ); j++ ) {
         l = i - j;
         at = A[j] + MUL_R(REAL32(rct[i - 1]),REAL32(A[l]));
         A[l] += (real_32_t)MUL_R(rct[i - 1],A[j]);
         A[j] = at;
      }
      A[i] = rct[i - 1];
      err += REAL2INT(rct[i - 1]*sum);

      if ( err <= 0 )
          err = 1;
   }
   memcpy( rc, rct, sizeof(real_32_t)<<2 );
   memcpy( old_A, A, sizeof(real_32_t)*MP1 );
}
 ...
 *
 */
static void lpc( real_32_t *old_A, real_t x[], real_t x_12k2[], 
                 real_32_t a[], enum Mode mode )
{
   Word32 i;
   int64_t r[MP1];
   real_32_t rc[4];
   ...
   if ( mode == MR122 ) {
       ...
       */
      for ( i = 1; i <= M; i++ ) {
         r[i] = REAL2INT(r[i] * lag_wind[i - 1]);
      }
      r[0] = REAL2INT(r[0]*REAL_CONST(1.0001));
 ...
 */
INLINE static real_t Chebps( real_t x, real_t f[] )
{
   real_t b0, b1, b2, x2;
   Word32 i;
   ...
   for ( i = 2; i < 5; i++ ) {
      b0 = MUL_R(x2,b1) - b2 + f[i];
      b2 = b1;
      b1 = b0;
   }
   return ( MUL_R(x,b1) - b2 + f[i] );
}
 ...
 *    void
 */
static void Az_lsp( real_32_t a[], real_32_t lsp[], real_32_t old_lsp[] )
{
   Word32 i, j, nf, ip;
   real_t xlow, ylow, xhigh, yhigh, xmid, ymid, xint;
   real_t y;
   real_t *coef;
   real_t f1[6], f2[6];
   ...
   while ( ( nf < M ) && ( j < 60 ) ) {
      ...
      if ( MUL_R(ylow,yhigh) <= REAL_ICONST(0) ) {
         ...
         if ( y == REAL_ICONST(0) ) {
            ...
            y = DIV_R((xhigh-xlow), (yhigh-ylow));
            xint = xlow - MUL_R(ylow,y);
         }
         lsp[nf] = xint;
         xlow = xint;
         nf++;
   ...
   if ( nf < M ) {
      memcpy( lsp, old_lsp, M*sizeof(real_32_t) );
   }
 ...
 *    void
 */
static void Get_lsp_pol( real_32_t *lsp, real_32_t *f )
{
   Word32 i, j;
   real_32_t T0;
   ...
   for ( i = 2; i <= 5; i++ ) {
      T0 = -(lsp[(i<<1) - 2]<<1);
      f[i] = MUL_R(T0,f[i - 1]) + (f[i - 2]<<1);

      for ( j = i - 1; j >= 2; j-- ) {
         f[j] = f[j] + MUL_R(T0,f[j - 1]) +f[j - 2];
      }
      f[1] = f[1] + T0;
   }
 ...
 *    void
 */
static void Lsp_Az( real_32_t lsp[], real_32_t a[] )
{
   real_32_t f1[6], f2[6];
   Word32 i, j;
 ...
 *    void
 */
static void Int_lpc_1and3_2( real_32_t lsp_old[], real_32_t lsp_mid[], 
                             real_32_t lsp_new[], real_32_t az[] )
{
   real_32_t lsp[M];
   Word32 i;

   for ( i = 0; i < M; i += 2 ) {
      lsp[i] = ( lsp_mid[i] + lsp_old[i] )>>1;
      lsp[i + 1] = ( lsp_mid[i + 1] +lsp_old[i+1] )>>1;
   }
   ...
   for ( i = 0; i < M; i += 2 ) {
      lsp[i] = ( lsp_mid[i] + lsp_new[i] )>>1;
      lsp[i + 1] = ( lsp_mid[i + 1] +lsp_new[i+1] )>>1;
   }
 ...
 *    void
 */
static void Lsp_lsf( real_32_t lsp[], real_32_t lsf[] )
{
   Word32 i;

   for ( i = 0; i < M; i++ ) {
      lsf[i] = fixed_acos(lsp[i]);
   }
 ...
 *    void
 */
static void Lsf_wt( real_32_t *lsf, real_32_t *wf )
{
   real_32_t temp;
   Word32 i;
   ...
   wf[9] = REAL_ICONST(4000) - lsf[8];

   for ( i = 0; i < 10; i++ ) {
      if ( wf[i] < REAL_ICONST(450) ) {
         temp = REAL_CONST(3.347) - MUL_R(SLOPE1_WGHT_LSF,wf[i]);
      }
      else {
         temp = REAL_CONST(1.8) - MUL_R(SLOPE2_WGHT_LSF,(wf[i] - REAL_ICONST(450)));
      }
      wf[i] = MUL_R(temp,temp);
   }
 ...
 *
 * Parameters:
 *    lsf_r1            B: 1st LSF residual vector
 *    lsf_r2            B: 2nd LSF residual vector
 ...
 */
static Word16 Vq_subvec( real_32_t *lsf_r1, real_32_t *lsf_r2, const real_32_t *dico,
                         real_32_t *wf1, real_32_t *wf2, Word16 dico_size )
{
   int32_t temp;
   int64_t dist, dist_min;
   const real_32_t *p_dico;
   Word32 i, index = 0;
   ...
   for ( i = 0; i < dico_size; i++ ) {
      temp = REAL2INT(lsf_r1[0] - *p_dico++);
      dist = temp*temp*REAL2INT(wf1[0]);
      temp = REAL2INT(lsf_r1[1] - *p_dico++);
      dist += temp*temp*REAL2INT(wf1[1]);
      temp = REAL2INT(lsf_r2[0] - *p_dico++);
      dist += temp*temp*REAL2INT(wf2[0]);
      temp = REAL2INT(lsf_r2[1] - *p_dico++);
      dist += temp*temp*REAL2INT(wf2[1]);
 ...
 *    index             quantization index
 */
static Word16 Vq_subvec_s(real_32_t *lsf_r1, real_32_t *lsf_r2, const real_32_t *dico,
                          real_32_t *wf1, real_32_t *wf2, Word16 dico_size)
{
   int64_t dist_min, dist1, dist2;
   int32_t temp1, temp2;
   const real_32_t *p_dico;
   Word32 i, index = 0;
   Word16 sign = 0;
   ...
   for ( i = 0; i < dico_size; i++ ) {
      temp1 = REAL2INT(lsf_r1[0] - *p_dico);
      temp2 = REAL2INT(lsf_r1[0] + *p_dico++);
      dist1 = temp1*temp1*REAL2INT(wf1[0]);
      dist2 = temp2*temp2*REAL2INT(wf1[0]);
      temp1 = REAL2INT(lsf_r1[1] - *p_dico);
      temp2 = REAL2INT(lsf_r1[1] + *p_dico++);
      dist1 += temp1*temp1*REAL2INT(wf1[1]);
      dist2 += temp2*temp2*REAL2INT(wf1[1]);
      temp1 = REAL2INT(lsf_r2[0] - *p_dico);
      temp2 = REAL2INT(lsf_r2[0] + *p_dico++);
      dist1 += temp1*temp1*REAL2INT(wf2[0]);
      dist2 += temp2*temp2*REAL2INT(wf2[0]);
      temp1 = REAL2INT(lsf_r2[1] - *p_dico);
      temp2 = REAL2INT(lsf_r2[1] + *p_dico++);
      dist1 += temp1*temp1*REAL2INT(wf2[1]);
      dist2 += temp2*temp2*REAL2INT(wf2[1]);
 ...
 *    void
 */
static void Reorder_lsf( real_32_t *lsf, real_32_t min_dist )
{
   real_32_t lsf_min;
   Word32 i;
 ...
 *    void
 */
static void Lsf_lsp( real_32_t lsf[], real_32_t lsp[] )
{
   Word32 i;

   for ( i = 0; i < M; i++ ) {
       lsp[i] = fixed_cos_pi_div_4k(lsf[i])>>FRAC2REAL_BIT;
   }
 ...
 *    index             quantization index
 */
static Word16 Vq_subvec3( real_32_t *lsf_r1, const real_32_t *dico, 
                          real_32_t *wf1, Word16 dico_size, Word32 use_half )
{
   int64_t dist, dist_min;
   int32_t temp;
   const real_32_t *p_dico;
   ...
   if ( use_half == 0 ) {
      for ( i = 0; i < dico_size; i++ ) {
         ...
         temp = REAL2INT(lsf_r1[1] - *p_dico++);
         dist += temp*temp*REAL2INT(wf1[1]);
         temp = REAL2INT(lsf_r1[2] - *p_dico++);
         dist += temp*temp*REAL2INT(wf1[2]);
 ...
 *    index             quantization index
 */
static Word16 Vq_subvec4( real_32_t *lsf_r1, const real_32_t *dico,
                          real_32_t *wf1, Word16 dico_size )
{
   int64_t dist, dist_min;
   int32_t temp;
   const real_32_t *p_dico;
   ...
   for ( i = 0; i < dico_size; i++ ) {
      ...
      temp = REAL2INT(lsf_r1[2] - *p_dico++);
      dist += temp*temp*REAL2INT(wf1[2]);
      temp = REAL2INT(lsf_r1[3] - *p_dico++);
      dist += temp*temp*REAL2INT(wf1[3]);
 ...
 *    void
 */
static void Q_plsf_3( enum Mode mode, real_t *past_rq,
                      real_32_t *lsp1, real_32_t *lsp1_q,
                      Word16 *indice, Word32 *pred_init_i )
{
   real_32_t lsf1[M], wf1[M], lsf_p[M], lsf_r1[M];
   real_32_t lsf1_q[M];
   real_t pred_init_err;
   real_t min_pred_init_err;
   real_t temp_r1[M];
   real_t temp_p[M];
   Word32 j, i;
   ...
   if ( mode != MRDTX ) {
      for ( i = 0; i < M; i++ ) {
         lsf_p[i] = mean_lsf_3[i] + MUL_R(past_rq[i],pred_fac[i]);
      ...
      *pred_init_i = 0;
      ...
      for ( j = 0; j < PAST_RQ_INIT_SIZE; j++ ) {
         ...
         for ( i = 0; i < M; i++ ) {
            ...
            pred_init_err += MUL_R(temp_r1[i],temp_r1[i]);
         }   /* next i */

         if ( pred_init_err < min_pred_init_err ) {
            min_pred_init_err = pred_init_err;
            memcpy( lsf_r1, temp_r1, M*sizeof(real_32_t) );
            memcpy( lsf_p, temp_p, M*sizeof(real_32_t) );
            memcpy( past_rq, &past_rq_init[j * M], M*sizeof(real_t) );
            *pred_init_i = j;
         }
      }
   }
   ...
   Reorder_lsf( lsf1_q, REAL_ICONST(50) );
 ...
 *    void
 */
static void Q_plsf_5( real_t *past_rq, real_32_t *lsp1, real_32_t *lsp2, 
                      real_32_t *lsp1_q, real_32_t *lsp2_q, Word16 *indice )
{
   real_32_t lsf1[M], lsf2[M], wf1[M], wf2[M], lsf_p[M], lsf_r1[M], lsf_r2[M];
   real_32_t lsf1_q[M], lsf2_q[M];
   Word32 i;
   ...
   for ( i = 0; i < M; i++ ) {
      ...
      lsf_p[i] = mean_lsf_5[i] + MUL_R(past_rq[i],REAL_CONST(0.65));
      lsf_r1[i] = lsf1[i] - lsf_p[i];
   ...
   Reorder_lsf( lsf1_q, REAL_ICONST(50) );
   Reorder_lsf( lsf2_q, REAL_ICONST(50) );
 ...
 *    void
 */
static void Int_lpc_1and3( real_32_t lsp_old[], real_32_t lsp_mid[], 
                           real_32_t lsp_new[], real_32_t az[] )
{
   Word32 i;
   real_32_t lsp[M];
 ...
 *    void
 */
static void Int_lpc_1to3_2( real_32_t lsp_old[], real_32_t lsp_new[], real_32_t az[] )
{
   real_32_t lsp[M];
   Word32 i;

   for ( i = 0; i < M; i += 2 ) {
      lsp[i] = (lsp_new[i]>>2) + (lsp_old[i]>>2)+(lsp_old[i]>>1);
      lsp[i + 1] = (lsp_new[i+1]>>2) + (lsp_old[i+1]>>2)+(lsp_old[i+1]>>1);
   }
   ...
   for ( i = 0; i < M; i += 2 ) {
      lsp[i] = ( lsp_old[i] + lsp_new[i] ) >> 1;
      lsp[i + 1] = ( lsp_old[i + 1] +lsp_new[i+1] ) >> 1;
   }
   ...
   for ( i = 0; i < M; i += 2 ) {
      lsp[i] = (lsp_old[i]>>2) + (lsp_new[i]>>2)+(lsp_new[i]>>1);
      lsp[i + 1] = (lsp_old[i + 1]>>2) + (lsp_new[i + 1]>>2)+(lsp_new[i + 1]>>1);
   }
 ...
 *    void
 */
static void Int_lpc_1to3( real_32_t lsp_old[], real_32_t lsp_new[], real_32_t az[] )
{
   real_32_t lsp[M];
   Word32 i;
 ...
 *    void
 */
static void lsp( enum Mode req_mode, enum Mode used_mode, real_32_t *lsp_old,
      real_32_t *lsp_old_q, real_t *past_rq, real_32_t az[], real_32_t azQ[], 
      real_32_t lsp_new[], Word16 **anap )
{
   real_32_t lsp_new_q[M];   /* LSPs at 4th subframe */
   real_32_t lsp_mid[M], lsp_mid_q[M];   /* LSPs at 2nd subframe */
   ...
   memcpy( lsp_old, lsp_new, M*sizeof(real_32_t) );
   memcpy( lsp_old_q, lsp_new_q, M*sizeof(real_32_t) );
}
 ...
 *    resonance flag
 */
static Word16 check_lsp( Word16 *count, real_32_t *lsp )
{
   real_t dist, dist_min1, dist_min2, dist_th;
   Word32 i;
   ...
   if ( lsp[1] > REAL_CONST(0.98) ) {
      dist_th = REAL_CONST(0.018);
   }
   else if ( lsp[1] > REAL_CONST(0.93) ) {
      dist_th = REAL_CONST(0.024);
   }
   else {
      dist_th = REAL_CONST(0.034);
   }

   if ( ( dist_min1 < REAL_CONST(0.046) ) || ( dist_min2 < dist_th ) ) {
      *count += 1;
   }
   else {
      *count = 0;
 ...
 *    void
 */
static void Weight_Ai( real_32_t a[], const real_t fac[], real_32_t a_exp[] )
{
   Word32 i;
 ...
 */
#if 0
static void Residu( real_t a[], real_t x[], real_t y[] )
{
    Word32 i,j;
#ifdef FIXED_USE_ASM
    int32_t hi,lo;
   ...
   for ( i = 0; i < L_SUBFR; i ++ ) {
#ifdef FIXED_USE_ASM
       FIXED_MUL(hi,lo,x[i],a[0]);
       for ( j = 1; j <= M; j++ )
           FIXED_MADD(hi,lo,REAL32(x[i-j]),REAL32(a[j]));
       y[i] = (real_t)FIXED_INT64_R(hi,lo);
#else
       y[i] = REAL_ICONST(0);
       for ( j = 0; j <= M; j++)
           y[i] += MUL_R(REAL32(x[i-j]),REAL32(a[j])); 
#endif
   }
...
static void Residu2( real_32_t a[], real_t x[], real_32_t y[] )
{
    Word32 i,j;
#ifdef FIXED_USE_ASM
    int32_t hi,lo;
   ...
   for ( i = 0; i < L_SUBFR; i ++ ) {
       ...
       for ( j = 1; j <= M; j++ )
           FIXED_MADD(hi,lo,REAL32(x[i-j]),REAL32(a[j]));
       y[i] = (real_32_t)FIXED_INT64_R(hi,lo);
#else
       y[i] = REAL_ICONST(0);
       for ( j = 0; j <= M; j++)
           y[i] += MUL_R(REAL32(x[i-j]),REAL32(a[j])); 
...
static void Residu3( real_32_t a[], real_t x[], real_t y[] )
{
    Word32 i,j;
#ifdef FIXED_USE_ASM
    int32_t hi,lo;
 ...
 *    void
 */
INLINE static void Syn_filt( real_t a[], real_t x[], real_t y[], 
                            real_t mem[], Word16 update )
{
   real_t tmp[50];
   real_t *yy;
   register Word32 i,j;
   ...
   /* Copy mem[] to yy[] */
   memcpy(tmp,mem,M*sizeof(real_t));
   yy=tmp+M;

   /* Do the filtering. */
   for ( i = 0; i < L_SUBFR; i++ ) {
#ifdef FIXED_USE_ASM
       FIXED_MUL(hi,lo,REAL32(x[i]),REAL32(a[0]));
       for (j=1;j<=M;j++)
           FIXED_MSUB(hi,lo,REAL32(a[j]),REAL32(yy[-j]));
       *yy++ = FIXED_INT64_R(hi,lo);
#else
       sum = MUL_R(REAL32(x[i]),REAL32(a[0]));
       for (j=1;j<=M;j++)
           sum -= MUL_R(REAL32(a[j]),REAL32(yy[-j]));
       *yy++ = sum;
#endif
       y[i] = yy[ - 1];
   }
   ...
   if ( update != 0 ) {
      memcpy(mem,y+30,M*sizeof(real_t));
   }

   return;
}

INLINE static void Syn_filt2( real_32_t a[], real_32_t x[], real_32_t y[], 
                            real_32_t mem[], Word16 update )
{
   real_32_t tmp[50];
   real_32_t *yy;
   register Word32 i,j;
   ...
   /* Copy mem[] to yy[] */
   memcpy(tmp,mem,M*sizeof(real_32_t));
   yy=tmp+M;

   /* Do the filtering. */
   for ( i = 0; i < L_SUBFR; i++ ) {
#ifdef FIXED_USE_ASM
       FIXED_MUL(hi,lo,x[i],REAL32(a[0]));
       for (j=1;j<=M;j++)
           FIXED_MSUB(hi,lo,REAL32(a[j]),yy[-j]);
       *yy++ = FIXED_INT64_R(hi,lo);
#else
       sum = MUL_R(x[i],a[0]);
       for (j=1;j<=M;j++)
           sum -= MUL_R(REAL32(a[j]),yy[-j]);
       *yy++ = sum;
#endif
       y[i] = yy[ - 1];
   }
   ...
   /* Update of memory if update==1 */
   if ( update != 0 ) {
      memcpy(mem,y+30,M*sizeof(real_32_t));
   }

   return;
}

INLINE static void Syn_filt3( real_32_t a[], real_t x[], real_t y[], 
                            real_t mem[], Word16 update )
{
   real_t tmp[50];
   real_t *yy;
 ...
 *    void
 */
static Word32 pre_big( enum Mode mode, 
                       real_32_t Ap1[], real_32_t Ap2[], 
                       Word16 frame_offset,
                       real_t speech[], real_32_t mem_w[], real_32_t wsp[] )
{
   Word32 i;
   Word16 offset;
   ...
   for ( i = 0; i < 2; i++ ) {
       ...
       *       i=1
       */
      Residu2( &Ap1[offset], &speech[frame_offset], &wsp[frame_offset] );
       ...
       *         i=1                           i=1
       */
      Syn_filt2( &Ap2[offset], &wsp[frame_offset], &wsp[frame_offset], mem_w, 1 );
 ...
 *    void
 */
static void comp_corr( real_32_t sig[], Word32 L_frame, Word32 lag_max, 
                       Word32 lag_min, int64_t corr[] )
{
    register Word32 i, j, k;
    real_32_t *p, *p1;
#ifdef FIXED_USE_ASM
    int32_t hi,lo;
#else
...
#ifdef FIXED_USE_ASM
          for ( k = 0; k <= 39; k++ )
              FIXED_MADD(hi,lo,REAL2INT32(p[k]),REAL2INT32(p1[k]));
#else
          for ( k = 0; k <= 39; k++ )
              T0 += REAL2INT32(p[k])*REAL2INT32(p1[k]);
#endif
      }
#ifdef FIXED_USE_ASM
      corr[ - i] = FIXED_INT64(hi,lo);
#else
      corr[ - i] = T0;
#endif
   }
 ...
 */
#ifndef VAD2
static void vad_tone_detection( vadState *st, int64_t T0, real_t t1 )
{
   if ( ( t1 > REAL_ICONST(0) ) && ( T0 > REAL2INT(MUL_R(t1,TONE_THR)) ) ) {
...
#else
static Word16 Lag_max( vadState *vadSt, int64_t corr[], real_32_t sig[], 
                       Word16 L_frame, Word32 lag_max, Word32 lag_min, 
                       real_t *cor_max, Word32 dtx )
#endif
{
   int64_t max;
   real_t T0;
   ...
   /* max = max/sqrt(energy) */
   if ( T0 > REAL_ICONST(0) )
      *cor_max = DIV_R(REAL_ICONST(max),T0);
   else
      *cor_max = REAL_ICONST(0);
 ...
 *    open loop pitch lag
 */
static Word32 Pitch_ol( enum Mode mode, vadState *vadSt, real_32_t signal[],
      Word32 pit_min, Word32 pit_max, Word16 L_frame, Word32 dtx, Word16 idx )
{
   int64_t corr[PIT_MAX + 1];
   real_t max1, max2, max3;
   Word16 p_max1, p_max2, p_max3;
   int64_t *corr_ptr;
   Word32 i, j;
   static real_t real_const1 = REAL_CONST(0.85);
#ifdef VAD2
   Float32 r01, r02, r03;
...
//   Float32 corr_hp_max;
   ...
   memset(corr,0,(PIT_MAX+1)*sizeof(int64_t));
   /*        79             */
   /* O(k) = SUM Sw(n)*Sw(n-k)   */
   /*        n=0               */
   ...
   /* Maxima 3 */
   i = j - 1;
   //Ken: Lag_max has precision problem???
   ...
   if ( MUL_R(max1,real_const1) < max2 ) {
      max1 = max2;
      p_max1 = p_max2;
#ifdef VAD2
      if (dtx) {
   ...
   if ( MUL_R(max1,real_const1) < max3 ) {
      p_max1 = p_max3;
#ifdef VAD2
      if (dtx) {
	rmax1 = rmax3;
 ...
 *    void
 */
static void ol_ltp( enum Mode mode, vadState *vadSt, real_32_t wsp[], 
                    Word32 *T_op, real_t ol_gain_flg[], Word32 *old_T0_med, 
                    Word16 *wght_flg, real_t *ada_w, Word32 *old_lags, 
                    Word32 dtx, Word16 idx )
{
   if ( mode != MR102 ) {
      ol_gain_flg[0] = REAL_ICONST(0);
      ol_gain_flg[1] = REAL_ICONST(0);
   }
 ...
 *    void
 */
static void subframePreProc( enum Mode mode, real_32_t Ap1[], real_32_t Ap2[], 
                             real_32_t *Aq, real_t *speech,
                             real_t *mem_err, real_t *mem_w0, real_t *zero,
                             real_t ai_zero[], real_t *exc, real_t h1[],
                             real_t xn[], real_t res2[], real_t error[] )
{
    ...
    */
    amr_real322real(Ap1,ai_zero,MP1);

    Syn_filt3( Aq, ai_zero, h1, zero, 0 );
    Syn_filt3( Ap2, h1, h1, zero, 0 );
   ...
   /* LP residual */
   Residu3( Aq, speech, res2 );
   memcpy( exc, res2, L_SUBFR*sizeof(real_t) );

   /* Synthesis filter */
   Syn_filt3( Aq, exc, error, mem_err, 0 );
   Residu3( Ap1, error, xn );

   /* target signal xn[] */
   Syn_filt3( Ap2, xn, xn, mem_w0, 0 );
}
 ...
 *    void
 */
static void Norm_Corr( real_t exc[], real_t xn[], 
                       real_t h[], Word32 t_min,
                       Word32 t_max, real_t corr_norm[] )
{
   real_t exc_temp[L_SUBFR];
   real_t *p_exc;
   int64_t corr, norm;
   register Word32 i, j, k;
#ifdef FIXED_USE_ASM
   ...
   for ( j = 0; j < L_SUBFR; j++ ) {
#ifdef FIXED_USE_ASM
      FIXED_MUL(hi,lo,REAL32(p_exc[0]),REAL32(h[j]));
      for ( i = 1; i <= j; i++ ) {
         FIXED_MADD(hi,lo,REAL32(p_exc[i]),REAL32(h[j - i]));
      }
      exc_temp[j] = FIXED_INT64_R(hi,lo);
#else
      sum = REAL_ICONST(0);

      for ( i = 0; i <= j; i++ ) {
         sum += MUL_R(REAL32(p_exc[i]),REAL32(h[j - i]));
      }
      exc_temp[j] = sum;
#endif
   }
   ...
   for ( i = t_min; i <= t_max; i++ ) {
      ...
      /*       n=0                     */
       norm = 0;
       for ( j=0; j<40; j++)
           norm += REAL2INT(fixed_fabs(exc_temp[j]));

      /*        39                  */
      /* SQRT[ SUM[ X(n) * Yk(n)] ] */
      /*       n=0                  */
      corr = Dotproduct40_fixed( xn, exc_temp );

      /* R(k) */
      if ( norm == 0 )
         corr_norm[i] = REAL_ICONST(corr);
      else
         corr_norm[i] = REAL_ICONST(corr)/norm;
      ...
      if ( i != t_max ) {
         ...
         for ( j = L_SUBFR - 1; j > 0; j-- ) {
            /* Yk(n) = Yk-1(n-1) + u(-k) * h(n) */
            exc_temp[j] = exc_temp[j - 1] + MUL_R(exc[k],h[j]);
         }
         exc_temp[0] = exc[k];
      }
   }
 ...
 */
static real_t Interpol_3or6( real_t *x, Word32 frac, Word16 flag3 )
{
   real_t s;
   real_t *x1, *x2;
   const real_t *c1, *c2;
   ...
   for ( i = 0, k = 0; i < L_INTER_SRCH; i++, k += UP_SAMP_MAX ) {
      /* R(k-i) * b24(t+i*6) */
      FIXED_MADD(hi,lo,REAL32(x1[ - i]),REAL32(c1[k]));

      /* R(k+1+i) * b24(6-t+i*6) */
      FIXED_MADD(hi,lo,REAL32(x2[i]),REAL32(c2[k]));
   }
   s = FIXED_INT64_R(hi,lo);
#else
   s = REAL_ICONST(0);
   for ( i = 0, k = 0; i < L_INTER_SRCH; i++, k += UP_SAMP_MAX ) {
      /* R(k-i) * b24(t+i*6) */
      s += MUL_R(REAL32(x1[ - i]),REAL32(c1[k]));

      /* R(k+1+i) * b24(6-t+i*6) */
      s += MUL_R(REAL32(x2[i]),REAL32(c2[k]));
   }
#endif
 ...
 *    void
 */
static void searchFrac( Word32 *lag, Word32 *frac, Word16 last_frac,
                        real_t corr[], Word16 flag3 )
{
   real_t max, corr_int;
   Word32 i;
 ...
 */
static Word32 Enc_lag3( Word32 T0, Word32 T0_frac, 
                        Word32 T0_prev, Word32 T0_min,
                        Word32 T0_max, Word16 delta_flag,
                        Word16 flag4 )
{
 ...
 *    index             index of encoding
 */
static Word32 Enc_lag6( Word32 T0, Word32 T0_frac, 
                        Word32 T0_min, Word16 delta_flag )
{
   Word32 index;
...
static Word32 Pitch_fr( Word32 *T0_prev_subframe, enum Mode mode, Word32 T_op[],
      real_t exc[], real_t xn[], real_t h[], Word16 i_subfr, Word32 *pit_frac,
      Word16 *resu3, Word32 *ana_index )
{
   ...
   corr = &corr_v[ - t_min]; //Ken: access invalid memory if t_min>0 ???
...
static void Pred_lt_3or6_fixed( Word32 exc[], Word32 T0, Word32 frac, Word32 flag3 )
{
   ...
   for ( i = 0; i < 40; i++ ) {
      ...
      hi = 0; lo = 0;
      for ( j=0; j<=9; j++) {
          FIXED_MADD(hi,lo,x1[-j],c1[j*6]);
          FIXED_MADD(hi,lo,x2[j],c2[j*6]);
      }
      s = FIXED_INT64(hi,lo);
#else
      s = 0;
 ...
 */
static real_t G_pitch( real_t xn[], real_t y1[], real_t gCoeff[] )
{
   real_t gain;
   real_t sum;

   /* Compute scalar product <y1[],y1[]> */
   sum = Dotproduct40_fix2( y1, y1 );

   /* Avoid case of all zeros */
   sum += REAL_CONST(0.01F);
   gCoeff[0] = sum;

   /* Compute scalar product <xn[],y1[]> */
   sum = Dotproduct40_fix2( xn, y1 );
   gCoeff[1] = sum;

   /* compute gain = xy/yy */
   gain = DIV_R( gCoeff[1],gCoeff[0] );

   /* if(gain >1.2) gain = 1.2 */
   if ( gain < REAL_ICONST(0) )
      gain = REAL_ICONST(0);

   if ( gain > REAL_CONST(1.2) )
      gain = REAL_CONST(1.2);
 ...
 *    True or false
 */
static Word16 check_gp_clipping( real_t *gp, real_t g_pitch )
{
   real_t sum;
   Word32 i;
   ...
   if ( sum > REAL_CONST(7.6F) /*FGP_CLIP*/ ) {
      return 1;
   }
   else {
      return 0;
 ...
 */
static Word16 q_gain_pitch( enum Mode mode, real_t gp_limit, 
                            real_t *gain, real_t gain_cand[],
                            Word32 gain_cind[] )
{
   real_t err_min, err;
   Word32 i, index;

   err_min = fixed_fabs( *gain - qua_gain_pitch[0] );
   index = 0;

   for ( i = 1; i < NB_QUA_PITCH; i++ ) {
      if ( qua_gain_pitch[i] <= gp_limit ) {
         err = fixed_fabs( *gain - qua_gain_pitch[i] );
 ...
 *    void
 */
static void cl_ltp( Word32 *T0_prev_subframe, real_t *gp, enum Mode mode,
                    Word16 frame_offset, Word32 T_op[], real_t *h1, 
                    real_t *exc, real_t res2[], real_t xn[], 
                    Word16 lsp_flag, real_t xn2[], real_t y1[],
                    Word32 *T0, Word32 *T0_frac, real_t *gain_pit,
                    real_t gCoeff[], Word16 **anap, real_t *gp_limit )
{
   Word32 i;
   Word32 n;
   ...
   *T0 = Pitch_fr( T0_prev_subframe, mode, T_op,
                   exc, xn, h1, frame_offset,
                   T0_frac, &resu3, &i );
   *( *anap )++ = ( Word16 )i;
    ...
    */
    for (i = -(PIT_MAX + L_INTERPOL); i < 40; i++)
      exc_tmp_p[i] = REAL2INT(exc[i]);

   Pred_lt_3or6_fixed( exc_tmp_p, *T0, *T0_frac, resu3 );

   for (i = -(PIT_MAX + L_INTERPOL); i < 40; i++)
      exc[i] = REAL_ICONST(exc_tmp_p[i]);
   ...
   for ( n = 0; n < L_SUBFR; n++ ) {
#ifdef FIXED_USE_ASM
      FIXED_MUL(hi,lo,exc[0],h1[n]);

      for ( i = 1; i <= n; i++ ) {
         FIXED_MADD(hi,lo,REAL32(exc[i]),REAL32(h1[n - i]));
      }
      y1[n] = FIXED_INT64_R(hi,lo);
#else
      s = REAL_ICONST(0);

      for ( i = 0; i <= n; i++ ) {
         s += MUL_R(REAL32(exc[i]),REAL32(h1[n - i]));
      }
      y1[n] = s;
#endif
   }
   ...
   *gp_limit = REAL_ICONST(2);
    ...
    */
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
      if ( *gain_pit > REAL_CONST(0.85F) ) {
         *gain_pit = REAL_CONST(0.85F);
      }
    ...
    */
   for ( i = 0; i < L_SUBFR; i++ ) {
      xn2[i] = xn[i] - MUL_R(y1[i],(*gain_pit));
      res2[i] = res2[i] - MUL_R(exc[i],(*gain_pit));
   }
}
...
INLINE static real_t DotProduct_fixed( real_t *x, real_t *y, Word32 len )
{
   Word32 i;
   real_t acc;
#ifdef FIXED_USE_ASM
...
static void cor_h_x_fixed( real_t h[], real_t x[], real_32_t dn[] )
{
   Word32 i;

   dn[0] = (real_32_t)Dotproduct40_fix2( h, x );

   for ( i = 1; i < L_CODE; i++ )
      dn[i] = (real_32_t)DotProduct_fixed( h, &x[i], L_CODE - i );
}
 ...
 *    void
 */
static void set_sign( real_32_t dn[], Word32 sign[], real_32_t dn2[], Word16 n )
{
   real_32_t val, min;
   Word32 i, j, k, pos = 0;
 ...
 *    void
 */
static void cor_h( real_t h[], Word32 sign[], real_32_t rr[][L_CODE] )
{
   real_32_t sum;
   real_32_t *prr;
   real_t *ph, *ph_max;
   ...
   rr[0][0] = (real_32_t)Dotproduct40_fix2( h, h );
   prr = &rr[39][39];
   ph = &h[0];
   ph_max = ph + 39;
 ...
 *    void
 */
static void search_4i40( real_32_t dn[], real_32_t dn2[],
                         real_32_t rr[][L_CODE], Word32 codvec[] )
{
   real_32_t alpk, alp, alp0, alp1;
   int32_t ps, ps0, ps1;
   int64_t sq, sq1, psk;
   Word32 ipos[4];
       ...
       */
         for ( i0 = ipos[0]; i0 < L_CODE; i0 += STEP ) {
            if ( dn2[i0] >= REAL_ICONST(0) ) {
               ...
               for ( i1 = ipos[1]; i1 < L_CODE; i1 += STEP ) {
                  ps1 = ps0 + REAL2INT(dn[i1]);

                  /* alp1 = alp0 + rr[i0][i1] + 1/2*rr[i1][i1]; */
                  alp1 = alp0 + (rr[i1][i1]>>2);
                  alp1 = alp1 + (rr[i0][i1]>>1);
                  sq1 = ps1*ps1;
               ...
               for ( i2 = ipos[2]; i2 < L_CODE; i2 += STEP ) {
                  ps1 = ps0 + REAL2INT(dn[i2]);

                  /* alp1 = alp0 + rr[i0][i2] + rr[i1][i2] + 1/2*rr[i2][i2]; */
                  alp1 = alp0 + (rr[i2][i2]>>4);
                  alp1 += (rr[i1][i2]>>3);
                  alp1 += (rr[i0][i2]>>3);
                  sq1 = ps1*ps1;
               ...
               for ( i3 = ipos[3]; i3 < L_CODE; i3 += STEP ) {
                  ps1 = ps0 + REAL2INT(dn[i3]);

                  /* alp1 = alp0 + rr[i0][i3] + rr[i1][i3] + rr[i2][i3] + 1/2*rr[i3][i3]; */
                  alp1 = alp0 + (rr[i3][i3]>>4);
                  alp1 += ((rr[i2][i3]+rr[i1][i3]+rr[i0][i3])>>3);
                  sq1 = ps1*ps1;
 ...
 *    void
 */
static void build_code_4i40( Word32 codvec[], Word32 dn_sign[], 
                             real_t cod[], real_t h[], 
                             real_t y[], Word16 *anap )
{
   real_t s;
   real_t *p0, *p1, *p2, *p3;
   Word32 _sign[4];
   ...
   memset( cod, 0, L_SUBFR*sizeof(real_t) );
   ...
   for ( k = 0; k < 4; k++ ) {
      ...
      if ( j > 0 ) {
         cod[i] = REAL_CONST(0.9998779296875F);
         _sign[k] = 1;
         rsign = rsign + ( 1 << track );
      }
      else {
 ...
 *    void
 */
static void code_4i40_17bits( real_t x[], real_t h[], 
                              Word32 T0, real_t pitch_sharp,
                              real_t code[], real_t y[], Word16 *anap )
{
   real_32_t rr[L_CODE][L_CODE];
   real_32_t dn[L_CODE], dn2[L_CODE];
   Word32 dn_sign[L_CODE];
   Word32 codvec[4];
   Word32 i;

   if ( ( T0 < L_CODE ) && ( pitch_sharp != 0 ) ) {
      for ( i = T0; i < L_CODE; i++ ) {
         h[i] = h[i] + MUL_R(h[i - T0],pitch_sharp);
      }
   }

   cor_h_x_fixed( h, x, dn );
   ...
   if ( ( T0 < L_CODE ) && ( pitch_sharp != 0 ) ) {
      for ( i = T0; i < L_CODE; i++ ) {
         code[i] = code[i] + MUL_R(code[i - T0],pitch_sharp);
      }
   }
}
 ...
 *    void
 */
static void set_sign12k2( real_32_t dn[], real_t cn[], 
                          Word32 sign[], Word32 pos_max[],
                          Word16 nb_track, Word32 ipos[], Word16 step )
{
   real_t b[L_CODE];
   real_t /*val,*/ cor, max, max_of_all;
   real_32_t val;
   real_t k_cn, k_dn;
   Word32 i, j, pos = 0;
   ...
   k_cn = REAL_ICONST(0);
   for (i=0;i<L_CODE;i++)
       k_cn += fixed_fabs(cn[i]);

   k_dn = REAL_ICONST(0);
   for (i=0;i<L_CODE;i++)
       k_dn += fixed_fabs(dn[i]);

   for ( i = 0; i < L_CODE; i++ ) {
      val = dn[i];
      cor = REAL_ICONST(0);
      if (k_cn>REAL_ICONST(0))
          cor += DIV_R(cn[i],k_cn);
      if (k_dn>REAL_ICONST(0))
          cor += DIV_R(val,k_dn);

      /* sign = +1 */
      sign[i] = 1;

      if ( cor < REAL_ICONST(0) ) {
         /* sign = -1 */
         sign[i] = -1;
         cor = -cor;
         val = -val;
 ...
 *    void
 */
static void build_code_8i40_31bits( Word32 codvec[], Word16 dn_sign[], Float32
 ...
 *    void
 */
static void search_10i40( real_32_t dn[], real_32_t rr[][L_CODE], Word32 ipos[],
      Word32 pos_max[], Word32 codvec[] )
{
   real_32_t rrv[L_CODE];
   int32_t ps, ps0, ps1, ps2;
   real_32_t alpk, alp, alp0, alp1, alp2;
   int64_t sq, sq2, psk;
   real_32_t *p_r, *p_r0, *p_r1, *p_r2, *p_r3, *p_r4, *p_r5, *p_r6, *p_r7, *p_r8,
         *p_r9, *p_r10;
   real_32_t *p_rrv, *p_rrv0;
   real_32_t *p_dn, *p_dn0, *p_dn1, *p_dn_max;
   ...
   for ( i = 1; i < 5; i++ ) {
      ...
      i8 = ipos[8];
      i9 = ipos[9];
      ps0 = REAL2INT(dn[i0]) + REAL2INT(dn[i1]);
      alp0 = *p_r + rr[i1][i1] + (rr[i0][i1]<<1);
      ...
      p_r1 = &rr[i1][i3];
      p_r3 = &rr[i3][i3];
      *p_rrv = *p_r3 + (( *p_r0 + *p_r1 )<<1);
      *( p_rrv + 5 ) = *( p_r3 + 205 ) + (( *( p_r0 + 5 ) + *( p_r1 + 5 ))<<1);
      *( p_rrv + 10 ) = *( p_r3 + 410 ) + (( *( p_r0 + 10 ) + *( p_r1 + 10 ))<<1);
      *( p_rrv + 15 ) = *( p_r3 + 615 ) + (( *( p_r0 + 15 ) + *( p_r1 + 15 ))<<1);
      *( p_rrv + 20 ) = *( p_r3 + 820 ) + (( *( p_r0 + 20 ) + *( p_r1 + 20 ))<<1);
      *( p_rrv + 25 ) = *( p_r3 + 1025 ) + (( *( p_r0 + 25 ) + *( p_r1 +25 ))<<1);
      *( p_rrv + 30 ) = *( p_r3 + 1230 ) + (( *( p_r0 + 30 ) + *( p_r1 +30 ))<<1);
      *( p_rrv + 35 ) = *( p_r3 + 1435 ) + (( *( p_r0 + 35 ) + *( p_r1 +35 ))<<1);
      sq = -1;
      alp = REAL_ICONST(1);
      ps = 0;
      ia = i2;
      ...
      do {
         ps1 = ps0 + REAL2INT(*p_dn0);
         alp1 = alp0 + *p_r2 + (( *p_r0 + *p_r1 )<<1);
         p_rrv = p_rrv0;
         p_dn = p_dn1;
         p_r4 = p_r3;

         do {
            ps2 = ps1 + REAL2INT(*p_dn);
            sq2 = ps2*ps2;
            alp2 = alp1 + *p_rrv + ((*p_r4)<<1);
      ...
      p_r3 = &rr[i3][i5];
      p_r5 = &rr[i5][i5];
      *p_rrv = *p_r5 + (( *p_r0 + *p_r1 + *p_r2 + *p_r3 )<<1);
      *( p_rrv + 5 ) = *( p_r5 + 205 ) + (( *( p_r0 + 5 ) + *( p_r1 + 5 )
            + *( p_r2 + 5 ) + *( p_r3 + 5 ) )<<1);
      *( p_rrv + 10 ) = *( p_r5 + 410 ) + (( *( p_r0 + 10 ) + *( p_r1 + 10
            ) + *( p_r2 + 10 ) + *( p_r3 + 10 ) )<<1);
      *( p_rrv + 15 ) = *( p_r5 + 615 ) + (( *( p_r0 + 15 ) + *( p_r1 + 15
            ) + *( p_r2 + 15 ) + *( p_r3 + 15 ) )<<1);
      *( p_rrv + 20 ) = *( p_r5 + 820 ) + (( *( p_r0 + 20 ) + *( p_r1 + 20
            ) + *( p_r2 + 20 ) + *( p_r3 + 20 ) )<<1);
      *( p_rrv + 25 ) = *( p_r5 + 1025 ) + (( *( p_r0 + 25 ) + *( p_r1 +
            25 ) + *( p_r2 + 25 ) + *( p_r3 + 25 ) )<<1);
      *( p_rrv + 30 ) = *( p_r5 + 1230 ) + (( *( p_r0 + 30 ) + *( p_r1 +
            30 ) + *( p_r2 + 30 ) + *( p_r3 + 30 ) )<<1);
      *( p_rrv + 35 ) = *( p_r5 + 1435 ) + (( *( p_r0 + 35 ) + *( p_r1 +
            35 ) + *( p_r2 + 35 ) + *( p_r3 + 35 ) )<<1);
      /* Default value */
      ps0 = ps;
      alp0 = alp;
      sq = -1;
      ...
      do {
         ps1 = ps0 + REAL2INT(*p_dn0);
         alp1 = alp0 + *p_r4 + (( *p_r0 + *p_r1 + *p_r2 + *p_r3 )<<1);
         p_dn = p_dn1;
         p_r6 = p_r5;
         p_rrv = p_rrv0;
      ...
      p_r5 = &rr[i5][i7];
      p_r7 = &rr[i7][i7];
      *p_rrv = *p_r7 + (( *p_r0 + *p_r1 + *p_r2 + *p_r3 + *p_r4 + *p_r5 )<<1);
      *( p_rrv + 5 ) = *( p_r7 + 205 ) + (( *( p_r0 + 5 ) + *( p_r1 + 5 )
            + *( p_r2 + 5 ) + *( p_r3 + 5 ) + *( p_r4 + 5 ) + *( p_r5 + 5 ) )<<1);
      *( p_rrv + 10 ) = *( p_r7 + 410 ) + (( *( p_r0 + 10 ) + *( p_r1 + 10
            ) + *( p_r2 + 10 ) + *( p_r3 + 10 ) + *( p_r4 + 10 ) + *( p_r5 + 10
            ) )<<1);
      *( p_rrv + 15 ) = *( p_r7 + 615 ) + (( *( p_r0 + 15 ) + *( p_r1 + 15
            ) + *( p_r2 + 15 ) + *( p_r3 + 15 ) + *( p_r4 + 15 ) + *( p_r5 + 15
            ) )<<1);
      *( p_rrv + 20 ) = *( p_r7 + 820 ) + (( *( p_r0 + 20 ) + *( p_r1 + 20
            ) + *( p_r2 + 20 ) + *( p_r3 + 20 ) + *( p_r4 + 20 ) + *( p_r5 + 20
            ) )<<1);
      *( p_rrv + 25 ) = *( p_r7 + 1025 ) + (( *( p_r0 + 25 ) + *( p_r1 +
            25 ) + *( p_r2 + 25 ) + *( p_r3 + 25 ) + *( p_r4 + 25 ) + *( p_r5 +
            25 ) )<<1);
      *( p_rrv + 30 ) = *( p_r7 + 1230 ) + (( *( p_r0 + 30 ) + *( p_r1 +
            30 ) + *( p_r2 + 30 ) + *( p_r3 + 30 ) + *( p_r4 + 30 ) + *( p_r5 +
            30 ) )<<1);
      *( p_rrv + 35 ) = *( p_r7 + 1435 ) + (( *( p_r0 + 35 ) + *( p_r1 +
      ...
      do {
         ps1 = ps0 + REAL2INT(*p_dn0);
         alp1 = alp0 + *p_r6 + (( *p_r0 + *p_r1 + *p_r2 + *p_r3 + *p_r4 +
               *p_r5 )<<1);
         p_dn = p_dn1;
         p_r8 = p_r7;
         p_rrv = p_rrv0;
      ...
      p_r7 = &rr[i7][i9];
      p_r9 = &rr[i9][i9];
      *p_rrv = *p_r9 + (( *p_r0 + *p_r1 + *p_r2 + *p_r3 + *p_r4 + *p_r5 +
            *p_r6 + *p_r7 )<<1);
      *( p_rrv + 5 ) = *( p_r9 + 205 ) + (( *( p_r0 + 5 ) + *( p_r1 + 5 )
            + *( p_r2 + 5 ) + *( p_r3 + 5 ) + *( p_r4 + 5 ) + *( p_r5 + 5 ) + *(
            p_r6 + 5 ) + *( p_r7 + 5 ) )<<1);
      *( p_rrv + 10 ) = *( p_r9 + 410 ) + (( *( p_r0 + 10 ) + *( p_r1 + 10
            ) + *( p_r2 + 10 ) + *( p_r3 + 10 ) + *( p_r4 + 10 ) + *( p_r5 + 10
            ) + *( p_r6 + 10 ) + *( p_r7 + 10 ) )<<1);
      *( p_rrv + 15 ) = *( p_r9 + 615 ) + (( *( p_r0 + 15 ) + *( p_r1 + 15
            ) + *( p_r2 + 15 ) + *( p_r3 + 15 ) + *( p_r4 + 15 ) + *( p_r5 + 15
            ) + *( p_r6 + 15 ) + *( p_r7 + 15 ) )<<1);
      *( p_rrv + 20 ) = *( p_r9 + 820 ) + (( *( p_r0 + 20 ) + *( p_r1 + 20
            ) + *( p_r2 + 20 ) + *( p_r3 + 20 ) + *( p_r4 + 20 ) + *( p_r5 + 20
            ) + *( p_r6 + 20 ) + *( p_r7 + 20 ) )<<1);
      *( p_rrv + 25 ) = *( p_r9 + 1025 ) + (( *( p_r0 + 25 ) + *( p_r1 +
            25 ) + *( p_r2 + 25 ) + *( p_r3 + 25 ) + *( p_r4 + 25 ) + *( p_r5 +
            25 ) + *( p_r6 + 25 ) + *( p_r7 + 25 ) )<<1);
      *( p_rrv + 30 ) = *( p_r9 + 1230 ) + (( *( p_r0 + 30 ) + *( p_r1 +
            30 ) + *( p_r2 + 30 ) + *( p_r3 + 30 ) + *( p_r4 + 30 ) + *( p_r5 +
            30 ) + *( p_r6 + 30 ) + *( p_r7 + 30 ) )<<1);
      *( p_rrv + 35 ) = *( p_r9 + 1435 ) + (( *( p_r0 + 35 ) + *( p_r1 +
            35 ) + *( p_r2 + 35 ) + *( p_r3 + 35 ) + *( p_r4 + 35 ) + *( p_r5 +
            35 ) + *( p_r6 + 35 ) + *( p_r7 + 35 ) )<<1);
      ...
      do {
         ps1 = ps0 + REAL2INT(*p_dn0);
         alp1 = alp0 + *p_r8 + (( *p_r0 + *p_r1 + *p_r2 + *p_r3 + *p_r4 +
               *p_r5 + *p_r6 + *p_r7 )<<1);
         p_dn = p_dn1;
         p_r10 = p_r9;
         p_rrv = p_rrv0;
 ...
 *    void
 */
static void build_code_10i40_35bits( Word32 codvec[], Word32 dn_sign[],
                                     real_t cod[], real_t h[], 
                                     real_t y[], Word16 indx[] )
{
   ...
   memset( cod, 0,  L_SUBFR*sizeof(real_t) );
   memset( y, 0,  L_SUBFR*sizeof(real_t) );
 ...
 *    void
 */
static void code_10i40_35bits( real_t x[], real_t cn[], real_t h[],
                               Word32 T0, real_t gain_pit, real_t code[],
                               real_t y[], Word16 anap[] )
 {
    real_32_t rr[L_CODE][L_CODE];
    real_32_t dn[L_CODE];
    ...
    /* include pitch contribution into impulse resp. */
    if ( gain_pit > REAL_ICONST(1) )
       gain_pit = REAL_ICONST(1);

    if ( gain_pit != REAL_ICONST(0) ) {
       for ( i = T0; i < L_SUBFR; i++ ) {
          h[i] += MUL_R(h[i - T0],gain_pit);
       }
    }
    /*
    *            39
    ...
    */
    ...
    if ( gain_pit != REAL_ICONST(0) ) {
       for ( i = T0; i < L_SUBFR; i++ ) {
          code[i] += MUL_R(code[i - T0],gain_pit);
       }
    }
    return;
}
 ...
 *    void
 */
static void cbsearch( enum Mode mode, Word16 subnr, real_t x[],
                     real_t h[], Word32 T0, real_t pitch_sharp,
                     real_t gain_pit, real_t code[], real_t y[],
                     real_t *res2, Word16 **anap )
{
   switch (mode){
/*   case MR475:
   case MR515:
 ...
 *    void
 */
static void gc_pred( Word32 *past_qua_en, enum Mode mode, real_t *code,
                     Word32 *gcode0_exp, Word32 *gcode0_fra, real_t *en )
{
   real_t ener_code;
   ...
   /* energy of code */
   ener_code = Dotproduct40_fix2( code, code );

   if ( mode == MR122 ) {
      ener = REAL2INT(ener_code)<<25;
      ...
      *gcode0_fra = ( ener >> 1 ) - ( *gcode0_exp << 15 );
      ...
      ener = ener_code<<(27-REAL_BITS);
      if (ener < 0)
         ener = 0x7fffffff;

      fixed_frexp(ener, &exp_code);
 ...
 *    void
 */
static void calc_filt_energies( enum Mode mode, real_t xn[], real_t xn2[],
                                real_t y1[], real_t y2[], real_t gCoeff[],
                                real_t coeff[], real_t *cod_gain )
{
   real_t sum, ener_init = REAL_CONST(0.01F);

   if ( ( mode == MR795 ) || ( mode == MR475 ) )
      ener_init = REAL_ICONST(0);

   coeff[0] = gCoeff[0];
   coeff[1] = -(gCoeff[1]<<1);

   /* Compute scalar product <y2[],y2[]> */
   sum = Dotproduct40_fix2( y2, y2 );
   sum += ener_init;
   coeff[2] = sum;

   /* Compute scalar product -2*<xn[],y2[]> */
   sum = Dotproduct40_fix2( xn, y2 );
   sum += ener_init;
   coeff[3] = -(sum<<1);

   /* Compute scalar product 2*<y1[],y2[]> */
   sum = Dotproduct40_fix2( y1, y2 );
   sum += ener_init;
   coeff[4] = (sum<<1);

   if ( ( mode == MR475 ) || ( mode == MR795 ) ) {
      /* Compute scalar product <xn2[],y2[]> */
      sum = Dotproduct40_fix2( xn2, y2 );

      if ( sum <= REAL_ICONST(0) ) {
         *cod_gain = REAL_ICONST(0);
      }
      else {
      /*
       * gcu = <xn2, y2> / <y2, y2>
       */
         *cod_gain = DIV_R(sum,coeff[2]);
      }
   }
}
 ...
 *    index             quantization index
 */
static Word16 q_gain_code( real_t gcode0, real_t *gain, Word32 *qua_ener_index)
{
   real_t err_min, err;
   const real_t *p;
   Word32 i, index;
   ...
   err_min = fixed_fabs( *gain - MUL_R(gcode0,*p++) );
   index = 0;

   for ( i = 1; i < NB_QUA_CODE; i++ ) {
      err = fixed_fabs( *gain - MUL_R(gcode0,*p++) );
   ...
   p = &gain_factor[index];
   *gain = fixed_floor(MUL_F(gcode0,*p));
 ...
 *    void
 */
static void MR795_gain_code_quant3( Word32 gcode0_exp, Word32 gcode0_fra, real_t g_pitch_cand[],
                                    Word32 g_pitch_cind[], real_t coeff[], real_t *gain_pit,
                                    Word32 *gain_pit_ind, real_t *gain_cod, Word32 *gain_cod_ind,
                                    Word32 *qua_ener_index )
{
   real_t gcode0, dist_min, g_pitch, g2_pitch, g_code, g_pit_cod, tmp0, tmp;
   int64_t g2_code;
   const real_t *p;
   Word32 i, j, cod_ind, pit_ind, g_code0, g_code_tmp;

   gcode0 = REAL_ICONST(Pow2( gcode0_exp, gcode0_fra));
   /*
   ...
   for ( j = 0; j < 3; j++ ) {
      ...
      g2_pitch = MUL_R(g_pitch,g_pitch);
      tmp0 = MUL_R(coeff[0],g2_pitch);
      tmp0 += MUL_R(coeff[1],g_pitch);
      p = &gain_factor[0];

      for ( i = 0; i < NB_QUA_CODE; i++ ) {
         /* this is g_fac */
         g_code = *p++;
         g_code = MUL_R(g_code,gcode0);
         g2_code = REAL2INT(g_code)*REAL2INT(g_code);
         g_pit_cod = MUL_R(g_code,g_pitch);
         tmp = tmp0;
		 tmp += coeff[2]*g2_code;
         tmp += MUL_R(coeff[3],g_code);
         tmp += MUL_R(coeff[4],g_pit_cod);
   ...
   *qua_ener_index = cod_ind;
   ...
   i = ( g_code_tmp * g_code0 ) << 1;
      ...
      if ( gcode0_exp > 0 )
         ...
         i = i << ( -gcode0_exp );
      *gain_cod = REAL_ICONST(i>>16);
     if (*gain_cod > REAL_ICONST(32767))
        *gain_cod = REAL_ICONST(32767);
 ...
 *    void
 */
static void calc_unfilt_energies( real_t res[], real_t exc[], real_t code[],
                                  real_t gain_pit, int64_t en[], real_t *ltpg )
{
   real_t sum;
   Word32 i;

   /* Compute residual energy */
   en[0] = Dotproduct40_fixed( res, res );
   ...
   en[1] = Dotproduct40_fixed( exc, exc );

   /* Compute scalar product <exc[],code[]> */
   en[2] = Dotproduct40_fixed( exc, code );
   ...
   for ( i = 0; i < L_SUBFR; i++ ) {
      /* LTP residual */
      sum = res[i] - MUL_R(exc[i],gain_pit);
      en[3] += REAL2INT(sum)*REAL2INT(sum);
   }

   /* calculate LTP coding gain, i.e. energy reduction LP res -> LTP res */
   if ( en[3] > 0 && en[0] != 0 ) {
      /* gain = ResEn / LTPResEn */
       *ltpg = fixed_log(REAL_ICONST(en[0]))-fixed_log(REAL_ICONST(en[3]));
   }
   else {
      *ltpg = REAL_ICONST(0);
   }
 ...
 *    index of the median value
 */
static real_t gmed_n_f( real_t ind[], Word16 n )
{
   Word32 medianIndex;
   Word32 i, j, ix = 0;
   Word32 tmp[9];
 ...
 *    void
 */
static void gain_adapt( real_t *prev_gc, Word16 *onset, real_t *ltpg_mem,
                        real_t *prev_alpha, real_t ltpg, real_t gain_cod, real_t *alpha )
{
   real_t result, filt;   /* alpha factor, median-filtered LTP coding gain */
   Word32 i;
   Word16 adapt;   /* adaptdation status; 0, 1, or 2 */

   /* basic adaptation */
   if ( ltpg <= REAL_CONST(0.3321928F) /*LTP_GAIN_THR1*/ ) {
      adapt = 0;
   }
   else {
      if ( ltpg <= REAL_CONST(0.6643856F) /*LTP_GAIN_THR2*/ ) {
         adapt = 1;
      }
      else {
         adapt = 2;
    ...
    * onset indicator:
    */
   if ( ( gain_cod > ((*prev_gc)<<1) ) && ( gain_cod > REAL_ICONST(100) ) ) {
      *onset = 8;
   }
   else {
      if ( *onset != 0 ) {
   ...
   if ( adapt == 0 ) {
      if ( filt > REAL_CONST(0.66443) ) {
         result = REAL_ICONST(0);
      }
      else {
         if ( filt < 0 ) {
            result = REAL_CONST(0.5);
         }
         else {
            result = REAL_CONST(0.5)-MUL_R(REAL_CONST(0.75257499),filt);
         }
      }
   }
   else {
 ...
 */
static Word16 MR795_gain_code_quant_mod( real_t gain_pit, Word32 gcode0_exp, Word32 gcode0_fra,
                                         int64_t en[], real_t alpha, real_t gain_cod_unq,
                                         real_t *gain_cod, Word32 *qua_ener_index )
{
   real_t coeff[5];
   real_t gcode0, g2_pitch, g_code, g2_code, d2_code, gain_code;
   real_t tmp,dist_min;
   const real_t *p;
   ...
   gain_code = *gain_cod;
   g2_pitch = MUL_R(gain_pit,gain_pit);
   coeff[0] = fixed_sqrt(alpha*en[0]);
   coeff[1] = MUL_R((alpha*en[1]),g2_pitch);
   coeff[2] = MUL_R((alpha*en[2]),gain_pit)<<1;
   coeff[3] = alpha*en[3];
   coeff[4] = REAL_ICONST(en[3]) - (alpha*en[3]);
   ...
   for ( i = 0; i < NB_QUA_CODE; i++ ) {
       ...
       */
      if ( g_code >= (gain_code<<1) )
         break;
      g2_code = MUL_R(g_code,g_code);
      d2_code = g_code - gain_cod_unq;
      d2_code = MUL_R(d2_code,d2_code);
      tmp = coeff[1] + MUL_R(coeff[2],g_code);
      tmp += MUL_R(coeff[3],g2_code);
      tmp = fixed_sqrt(tmp);
      tmp = tmp - coeff[0];
      tmp = MUL_R(tmp,tmp);
      tmp += MUL_R(coeff[4],d2_code);
 ...
 *    gcode0_exp/gcode0_frac            I: predicted CB gain
 *    cod_gain          I: codebook gain
 *    gp_limit          I: pitch gain limit
 *    gain_pit          B: Pitch gain
 *    gain_cod          O: Code gain
 *    qua_ener_index    O: quantized energy error_index
 ...
 *    void
 */
static void MR795_gain_quant( real_t *prev_gc, Word16 *onset, real_t *ltpg_mem,
                              real_t *prev_alpha, real_t res[], real_t exc[],
                              real_t code[], real_t coeff[], real_t code_en,
                              Word32 gcode0_exp, Word32 gcode0_fra, real_t cod_gain,
                              real_t gp_limit, real_t *gain_pit, real_t *gain_cod,
                              Word32 *qua_ener_index, Word16 **anap )
{
   int64_t en[4];
   real_t g_pitch_cand[3];
   real_t ltpg, alpha, gain_cod_unq;   /* code gain (unq.) */
   ...
   if ( ( en[0] != 0 ) && ( alpha > REAL_ICONST(0) ) ) {
      ...
      /* run quantization with mod2048 * *pified criterion */
 ...
 *    index             index of quantization
 */
static void gainQuant( enum Mode mode, Word32 even_subframe, Word32 *past_qua_en,
                       Word32 *past_qua_en_unq, real_t *sf0_coeff, 
                       real_t *sf0_target_en, Word32 *sf0_gcode0_exp,
                       Word32 *sf0_gcode0_fra,Word16 **gain_idx_ptr,
                       real_t *sf0_gain_pit, real_t *sf0_gain_cod, 
                       real_t *res, real_t *exc, real_t code[],
                       real_t xn[], real_t xn2[], real_t y1[],
                       real_t y2[], real_t gCoeff[],
                       real_t gp_limit, real_t *gain_pit,
                       real_t *gain_cod, real_t *prev_gc,
                       Word16 *onset, real_t *ltpg_mem,
                       real_t *prev_alpha, Word16 **anap )
{
   real_t coeff[5];
   real_t gcode0, cod_gain, en = 0;
   Word32 i, exp, frac, qua_ener_index = 0;
   ...
   if ( mode == MR475 ) {
      ...
      if ( mode == MR122 ) {
         ...
         gcode0 = REAL_ICONST(Pow2( exp, frac ));
         /* saturation at decoder */
         if (gcode0 > REAL_CONST(2047.9375F) )
             gcode0 = REAL_CONST(2047.9375F);

		 *gain_cod = REAL_ICONST(Dotproduct40_fixed (xn2, y2))/Dotproduct40_fixed (y2,y2);

         if ( *gain_cod < REAL_ICONST(0) )
            *gain_cod = REAL_ICONST(0);
 ...
 *    void
 */
static void subframePostProc( real_t *speech, Word16 i_subfr, real_t gain_pit,
                              real_t gain_code, real_32_t *a_q, real_t synth[],
                              real_t xn[], real_t code[], real_t y1[],
                              real_t y2[], real_t *mem_syn, real_t *mem_err,
                              real_t *mem_w0, real_t *exc, real_t *sharp )
{
   Word32 i, j;
   static real_t real_const1 = REAL_CONST(0.5);
   static real_t real_const2 = REAL_CONST(0.794556F);

   /*
    * Update pitch sharpening "sharp" with quantized gain_pit
    */
   *sharp = gain_pit;
   if ( *sharp > real_const2 ) {
      *sharp = real_const2;
   }

   /* Find the total excitation */
   for ( i = 0; i < L_SUBFR; i += 4 ) {
      exc[i + i_subfr] = fixed_floor(MUL_R(gain_pit,exc[i + i_subfr]) + 
                                     MUL_R(gain_code,code[i]) + real_const1);
      exc[i + i_subfr + 1] = fixed_floor(MUL_R(gain_pit,exc[i + i_subfr + 1]) +
                                         MUL_R(gain_code,code[i + 1]) + real_const1);
      exc[i + i_subfr + 2] = fixed_floor(MUL_R(gain_pit,exc[i + i_subfr + 2]) + 
                                         MUL_R(gain_code,code[i + 2]) + real_const1);
      exc[i + i_subfr + 3] = fixed_floor(MUL_R(gain_pit,exc[i + i_subfr + 3]) + 
                                         MUL_R(gain_code,code[i + 3]) + real_const1);
   }

   /* The local synthesis speech */
   Syn_filt3( a_q, &exc[i_subfr], &synth[i_subfr], mem_syn, 1 );
   ...
   for ( i = L_SUBFR - M, j = 0; i < L_SUBFR; i++, j++ ) {
      ...
      /* ew(n) = x(n) - Gp_q * y(n) - Gc_q(n) * z(n) */
      mem_w0[j] = xn[i] - 
                  MUL_R(REAL32(y1[i]),REAL32(gain_pit)) -
                  MUL_R(REAL32(y2[i]),REAL32(gain_code));
   }
}
 ...
 *    void
 */
static void dtx_buffer( Word16 *hist_ptr, real_t *lsp_hist, real_t lsp_new[],
                        real_t speech[], real_t *log_en_hist )
{
   int64_t frame_en;
   ...
   *hist_ptr += 1;
   ...
   memcpy( &lsp_hist[ (*hist_ptr) * M], lsp_new, sizeof( real_t )*M );

   /* compute log energy based on frame energy */
   frame_en = Dotproduct40_fixed( speech, speech );
   frame_en += Dotproduct40_fixed( &speech[40], &speech[40] );
   frame_en += Dotproduct40_fixed( &speech[80], &speech[80] );
   frame_en += Dotproduct40_fixed( &speech[120], &speech[120] );

   if ( frame_en > REAL_ICONST(1) ) {
      log_en_hist[*hist_ptr] = MUL_R(fixed_log10(frame_en)
                                     +REAL_BITS*fixed_log10(REAL_ICONST(2))
                                     -REAL_CONST(2.20411998),
                                     REAL_CONST(1.660964));
   }
   else {
      log_en_hist[*hist_ptr] = REAL_CONST(-3.660965F);
   }
}
 ...
 *    void
 */
static void cod_amr( cod_amrState *st, enum Mode mode, real_t new_speech[],
                     Word16 ana[], enum Mode *used_mode, real_t synth[] )
{
   /* LPC coefficients */
   real_32_t A_t[MP1<<2];   /* A(z) unquantized for the 4 subframes */
   real_32_t Aq_t[MP1<<2];  /* A(z)   quantized for the 4 subframes */
   real_32_t *Aq;           /* Pointer on Aq_t */
   real_32_t lsp_new[M];

   /* Other vectors */
   real_t xn[L_SUBFR];   /* Target vector for pitch search */
   real_t xn2[L_SUBFR];   /* Target vector for codebook search */
   real_t code[L_SUBFR];   /* Fixed codebook excitation */
   real_t y1[L_SUBFR];   /* Filtered adaptive excitation */
   real_t y2[L_SUBFR];   /* Filtered fixed codebook excitation */
   real_t gCoeff[3];   /* Correlations between xn, y1, & y2: */
   real_t res[L_SUBFR];   /* Short term (LPC) prediction residual */
   real_t res2[L_SUBFR];   /* Long term (LTP) prediction residual */

   /* Vector and scalars needed for the MR475 */
//   real_t xn_sf0[L_SUBFR];   /* Target vector for pitch search */
//   real_t y2_sf0[L_SUBFR];   /* Filtered codebook innovation */
//   real_t code_sf0[L_SUBFR];   /* Fixed codebook excitation */
//   real_t h1_sf0[L_SUBFR];   /* The impulse response of sf0 */
//   real_t mem_syn_save[M];   /* Filter memory */
///   real_t mem_w0_save[M];   /* Filter memory */
//   real_t mem_err_save[M];   /* Filter memory */
//   real_t sharp_save = REAL_ICONST(0);   /* Sharpening */
   real_t gain_pit_sf0;   /* Quantized pitch gain for sf0 */
   real_t gain_code_sf0;   /* Quantized codebook gain for sf0 */
//   Word16 i_subfr_sf0 = 0;   /* Position in exc[] for sf0 */

   /* Scalars & Flags */
   real_t gain_pit, gain_code;
   real_t gp_limit;   /* pitch gain limit value */
...
//   Word16 vad_flag;
//   Word16 *ana_saved = ana;

   real_32_t Ap1[MP1<<2],Ap2[MP1<<2];
   const real_t *g1;
   Word16 offset;

   memcpy( st->new_speech, new_speech, L_FRAME*sizeof(real_t));
    ...
    */
   /* LP analysis */
   memset(A_t,0,sizeof(real_32_t)*(MP1<<2));
   lpc( st->lpcSt->LevinsonSt->old_A, st->p_window, st->p_window_12k2, A_t, mode);
#ifdef AMR_DEBUG_LPC
   amr_dump_real32("cod_amr::old_A",st->lpcSt->LevinsonSt->old_A,MP1);
   amr_dump_real32("cod_amr::A_t",A_t,(MP1<<2));
#endif
...
#ifdef AMR_DEBUG_LSP
   amr_dump_real32("cod_amr::lsp_old",st->lspSt->lsp_old,M);
   amr_dump_real32("cod_amr::lsp_old_q",st->lspSt->lsp_old_q,M);
   amr_dump_real("cod_amr::past_rq",st->lspSt->qSt->past_rq,M);
   amr_dump_real32("cod_amr::A_t",A_t,MP1<<2);
   amr_dump_real32("cod_amr::Aq_t",Aq_t,MP1<<2);
   amr_dump_real32("cod_amr::lsp_new",lsp_new,M);
	
   if ( *used_mode == MR122 )
       amr_dump_int16("cod_amr::ana",ana-5,5);
   else
       amr_dump_int16("cod_amr::ana",ana-3,3);
#endif
   ...
   for ( i = 0; i < 4; i++) {
       /* a(i) * g1(i) */
       Weight_Ai( &A_t[offset], g1, &Ap1[i*MP1] );

       /* a(i) * g2(i) */
       Weight_Ai( &A_t[offset], gamma2, &Ap2[i*MP1] );

       offset += MP1;
   }

#ifdef AMR_DEBUG_WEIGHT
   amr_dump_real32("cod_amr::Ap1",Ap1,MP1<<2);
   amr_dump_real32("cod_amr::Ap2",Ap2,MP1<<2);
#endif
   ...
   for ( subfrNr = 0, i_subfr = 0; subfrNr < 2; subfrNr++, i_subfr += L_FRAME_BY2 ) {
       ...
       */
      pre_big( mode, &Ap1[offset], &Ap2[offset], 
               i_subfr, st->speech, st->mem_w, st->wsp );
      offset += (MP1<<1);

      /* Find open loop pitch lag for two subframes */
      if ( ( mode != MR475 ) && ( mode != MR515 ) ) {
         ol_ltp( mode, st->vadSt, &st->wsp[i_subfr], &T_op[subfrNr], 
             st->ol_gain_flg, &st->pitchOLWghtSt->old_T0_med, 
             &st->pitchOLWghtSt->wght_flg, &st->pitchOLWghtSt->ada_w, 
             st->old_lags, st->dtx,subfrNr );
      }
   }
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
      /*
      ...
      ol_ltp( mode, st->vadSt, &st->wsp[0], &T_op[0], st->ol_gain_flg, 
          &st->pitchOLWghtSt->old_T0_med, &st->pitchOLWghtSt->wght_flg, 
          &st->pitchOLWghtSt->ada_w, st->old_lags, st->dtx, 1 );
      T_op[1] = T_op[0];
   }

#ifdef AMR_DEBUG_PREBIG
   amr_dump_real32("cod_amr::st->wsp",st->wsp,L_FRAME);
#endif
#ifdef AMR_DEBUG_OL_LTP
   amr_dump_int32("cod_amr::T_op",T_op,2);
#endif
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      if ( *used_mode != MR475 ) {
         subframePreProc( *used_mode, &Ap1[offset], &Ap2[offset], 
               Aq, &st->speech[i_subfr], st->mem_err, st->mem_w0, st->zero,
               st->ai_zero, &st->exc[i_subfr], st->h1, xn, res, st->error );
         offset += MP1;
      }
#if 0
      /* MR475 */
...
#ifdef AMR_DEBUG_SUBFRAMEPREPROC
      amr_dump_real("cod_amr:st->ai_zero",st->ai_zero,MP1);
      amr_dump_real("cod_amr::st->exc",&st->exc[i_subfr],L_SUBFR);
      amr_dump_real("cod_amr::st->h1",st->h1,L_SUBFR);
      amr_dump_real("cod_amr::xn",xn,L_SUBFR);
      amr_dump_real("cod_amr::res",res,L_SUBFR);
      amr_dump_real("cod_amr::st->error",st->error,L_SUBFR);
#endif

      /* copy the LP residual (res2 is modified in the CL LTP search) */
      memcpy( res2, res, L_SUBFR*sizeof(real_t) );
...
#ifdef AMR_DEBUG_CL_LTP
      amr_dump_int32("cod_amr::T0_prev_subframe",&st->clLtpSt->pitchSt->T0_prev_subframe,1);
      amr_dump_real("cod_amr::exc",&st->exc[i_subfr],L_SUBFR);
      amr_dump_real("cod_amr::res2",res2,L_SUBFR);
      amr_dump_real("cod_amr::xn2",xn2,L_SUBFR);
      amr_dump_real("cod_amr::y1",y1,L_SUBFR);
      amr_dump_int32("cod_amr::T0",&T0,1);
      amr_dump_int32("cod_amr::T0_frac",&T0_frac,1);
      amr_dump_real("cod_amr::gain_pit",&gain_pit,1);
      amr_dump_real("cod_amr::gCoeff",gCoeff,2);
      if ( *used_mode == MR122 )
          amr_dump_int16("cod_amr::ana",ana-2,2);
      else
          amr_dump_int16("cod_amr::ana",ana-1,1);
      amr_dump_real("cod_amr::gp_limit",&gp_limit,1);
#endif
...
#ifdef AMR_DEBUG_CBSEARCH
      amr_dump_real("cod_amr::st->h1",st->h1,L_SUBFR);
      amr_dump_real("cod_amr::code",code,L_SUBFR);
      amr_dump_real("cod_amr::y2",y2,L_SUBFR);
      if ( *used_mode == MR795 )
          amr_dump_int16("cod_amr::ana",ana-2,2);
#endif
      ...
      gainQuant( *used_mode, evenSubfr, st->gainQuantSt->gc_predSt->past_qua_en,
            ...
            st->gainQuantSt->sf0_coeff, &st->gainQuantSt->sf0_target_en, 
            &st->gainQuantSt->sf0_gcode0_exp, &st->gainQuantSt->sf0_gcode0_fra, 
            &st->gainQuantSt->gain_idx_ptr, &gain_pit_sf0, &gain_code_sf0,
            res, &st->exc[i_subfr], code, xn, xn2, y1, y2, gCoeff, gp_limit,
            &gain_pit, &gain_code, &st->gainQuantSt->adaptSt->prev_gc,
            &st->gainQuantSt->adaptSt->onset, st->gainQuantSt->adaptSt->ltpg_mem,
...
#ifdef AMR_DEBUG_GAINQUANT
	  amr_dump_real32("cod_amr::gain_pit",&gain_pit,1);
	  amr_dump_real32("cod_amr::gain_code",&gain_code,1);
	  amr_dump_real32("cod_amr::prev_gc",&st->gainQuantSt->adaptSt->prev_gc,1);
	  amr_dump_int16("cod_amr::onset",&st->gainQuantSt->adaptSt->onset,1);
	  amr_dump_real32("cod_amr::ltpg_mem",st->gainQuantSt->adaptSt->ltpg_mem,LTPG_MEM_SIZE);
	  amr_dump_real32("cod_amr::prev_alpha",&st->gainQuantSt->adaptSt->prev_alpha,1);
	  if ( *used_mode == MR795 )
		  amr_dump_int16("cod_amr::ana",ana-2,2);
#endif
...
#ifdef AMR_DEBUG_SUBFRAMEPOSTPROC
	  amr_dump_real32("subframePostProc::synth",synth,L_FRAME);
      amr_dump_real32("subframePostProc::mem_syn",st->mem_syn,M);
      amr_dump_real32("subframePostProc::mem_err",st->mem_err,M);
      amr_dump_real32("subframePostProc::mem_w0",st->mem_w0,M);
#endif
...
the_end:
   ...
   memcpy(st->old_wsp,st->old_wsp+L_FRAME,PIT_MAX*sizeof(real_t));
   memcpy(st->old_exc,st->old_exc+L_FRAME,(PIT_MAX+L_INTERPOL)*sizeof(real_t));
   memcpy(st->old_speech,st->old_speech+L_FRAME,(L_TOTAL-L_FRAME)*sizeof(real_t));
}
 ...
 *    void
 */
static void Pre_Process( real_t *y2, real_t *y1, Word16 *x0,
                         Word16 *x1, Word16 *speech, real_t *f_speech )
{
   Word32 i;
   Word16 x2;
   real_t tmp;
   static frac_t real_const1 = FRAC_CONST(0.4636230465);
   static frac_t real_const2 = FRAC_CONST(0.92724705);
   static frac_t real_const3 = FRAC_CONST(0.4636234515);
   static frac_t real_const4 = FRAC_CONST(1.906005859);
   static frac_t real_const5 = FRAC_CONST(0.911376953);
   static frac_t real_const6 = FRAC_CONST(0.0000000001);

   for ( i = 0; i < 160; i++ ) {
      x2 = *x1;
      *x1 = *x0;
      *x0 = speech[i];
      tmp = ((real_const1 * *x0 -  real_const2 * *x1 + real_const3 * x2)>>FRAC2REAL_BIT)
			+ MUL_F(real_const4,*y1) -  MUL_F(real_const5,*y2); 
      f_speech[i] = tmp;
      *y2 = *y1;
      *y1 = tmp;
   }

   if ( ( fixed_fabs( *y1 )+fixed_fabs( *y2 ) ) < real_const6 ) //Ken: may have bug here.precision too low
      *y2 = *y1 = REAL_ICONST(0);
}
...
static void cod_amr_reset( cod_amrState *s, Word32 dtx )
{
   ...
   /* reset Q_plsfState */
   memset( s->lspSt->qSt->past_rq, 0, sizeof( real_t )*M );
   ...
   s->gainQuantSt->adaptSt->prev_alpha = REAL_ICONST(0);
   s->gainQuantSt->adaptSt->prev_gc = REAL_ICONST(0);
   memset( s->gainQuantSt->adaptSt->ltpg_mem, 0, sizeof( real_t )*LTPG_MEM_SIZE
         );
   s->gainQuantSt->sf0_gcode0_exp = 0;
   s->gainQuantSt->sf0_gcode0_fra = 0;
   s->gainQuantSt->sf0_target_en = REAL_ICONST(0);
   memset( s->gainQuantSt->sf0_coeff, 0, sizeof( real_t )*5 );
   s->gainQuantSt->gain_idx_ptr = NULL;

   /* reset pitchOLWghtState */
   s->pitchOLWghtSt->old_T0_med = 40;
   s->pitchOLWghtSt->ada_w = REAL_ICONST(0);
   s->pitchOLWghtSt->wght_flg = 0;

   /* reset tonStabState */
   s->tonStabSt->count = 0;
   memset( s->tonStabSt->gp, 0, sizeof( real_t )*N_FRAME );

   /* reset LevinsonState */
   s->lpcSt->LevinsonSt->old_A[0] = REAL_ICONST(1);
   memset( &s->lpcSt->LevinsonSt->old_A[1], 0, sizeof(real_32_t)*M );
   ...
   for ( i = 0; i < DTX_HIST_SIZE; i++ ) {
      memcpy( &s->dtxEncSt->lsp_hist[i * M], lsp_init_data, sizeof( real_t )*M);
   }
   memset( s->dtxEncSt->log_en_hist, 0, M * sizeof( real_t ) );
   ...
   memset( s->old_speech, 0, sizeof( real_t )*L_TOTAL );
   memset( s->old_exc, 0, sizeof( real_t )*( PIT_MAX + L_INTERPOL ) );
   memset( s->old_wsp, 0, sizeof( real_t )*PIT_MAX );
   memset( s->mem_syn, 0, sizeof( real_t )*M );
   memset( s->mem_w, 0, sizeof( real_t )*M );
   memset( s->mem_w0, 0, sizeof( real_t )*M );
   memset( s->mem_err, 0, sizeof( real_t )*M );
   memset( s->ai_zero, 0, sizeof( real_t )*L_SUBFR );
   memset( s->hvec, 0, sizeof( real_t )*L_SUBFR );

   for ( i = 0; i < 5; i++ ) {
      s->old_lags[i] = 40;
   }
   s->sharp = REAL_ICONST(0);
}
...
void * Speech_Encode_Frame_init( int dtx )
{
   ...
   if ( Pre_Process_init( &s->pre_state ) || 
        cod_amr_init( &s->cod_amr_state, dtx ) ) {
      Speech_Encode_Frame_exit( &s1 );
      return NULL;
   }
   return s;
}
...
{
   real_t syn[L_FRAME];   /* Buffer for synthesis speech */
   real_t speech[160];
   Word32 i;
   ...
   Pre_Process( &state->pre_state->y2, &state->pre_state->y1, 
                &state->pre_state->x0, &state->pre_state->x1,
                new_speech, speech );

#ifdef AMR_DEBUG_PREPROCESS
   amr_dump_real32("Speech_Encode_Frame::speech",speech,160);
#endif

   memset(syn,0,sizeof(real_t)*L_FRAME);
