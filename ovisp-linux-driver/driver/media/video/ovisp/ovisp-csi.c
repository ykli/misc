...
#include "ovisp-csi.h"
#include <mach/ovisp-pmic.h>

//#define OVISP_CSI_DEBUG
#ifdef OVISP_CSI_DEBUG
...
static uint8_t csi_phy_read(void)
{
	return (csi_reg_readl(CTL_CPHY0_CTRL1)& 0xFF00)>>8;
}
...
static int csi_phy_ready(unsigned int id)
{
	...
	// TODO: phy0: lane0 & lane1 is ready. need to be update for other lane
...
int csi_phy_start(unsigned int id, unsigned int freq)
{
	...
	pmic_voltage_set(PMIC_POWER_CAMERA_CSI_PHY, id, PMIC_POWER_VOLTAGE_ENABLE);
...
int csi_phy_stop(unsigned int id)
{
	...
	csi_phy_test_clear(id, 1);
	pmic_voltage_set(PMIC_POWER_CAMERA_CSI_PHY, id, PMIC_POWER_VOLTAGE_DISABLE);
	return 0;
}
