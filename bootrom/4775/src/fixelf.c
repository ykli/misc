...
get_data (void *var, FILE *file, long offset, size_t size, size_t nmemb)
{
  ...
  if (fread (mvar, size, nmemb, file) != nmemb)
    {
      printf ("Unable to read in 0x%lx bytes\n", (unsigned long)(size * nmemb));
      if (mvar != var)
        free (mvar);
      return NULL;
    }
...
//printf ("e_shnum = %d, phnum = %d, ndx = %d \n", elf_header.e_shnum, elf_header.e_phnum, elf_header.e_shstrndx);
    ...
    if (! elf_header.e_shoff) {
      printf ("e_shoff of elf_header is zero\n");
      goto err_ret;
    }
...
// replace vale
   ...
   if (argc == 4)
   {
     ...
     fwrite (&val, 4, 1, file);
     printf ("sym name = %s, file off = 0x%x, size = 0x%x, replace: val = 0x%08lx\n", argv[2], fileoff, symsize, val);
   }
