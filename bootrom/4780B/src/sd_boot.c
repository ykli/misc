...
/* reset emmc card by rst_n */
static inline void jz_rst_emmc(void)
{
	REG_MSC_CTRL2(ctl_num) &= ~(MSC_CTRL2_RST_EN);    // H/W reset
	xudelay(100);                               // min 1us, do not include rising and falling edge
	REG_MSC_CTRL2(ctl_num) |= (MSC_CTRL2_RST_EN);
	xudelay(200);                               // min 200us, host can issue boot initiation or cmd1
}
...
int sd_boot(num)
{
	...
	ret = mmc_block_read(SPL_SIZE, (u32 *)START_ADDR);	//SDRAM ADDR

	if(!ret)
	{
		if (!(REG32(START_ADDR + 512) == 0x4d53504c)) {
			if (ctl_num == 0)
				return sd_boot(1);
			if (ctl_num == 1)
				return usb_boot();
		} else {
			return xfer_d2i(start_addr + 516, SPL_SIZE);
		}
	} else {
		return error_handler(current_boot);
	}
...
int emmc_boot(void)
{
	...
	mmc_cmd(0, 0xfffffffa, 0x0, MSC_CMDAT_RESPONSE_NONE);

	REG_MSC_CMDAT(ctl_num) = ((MSC_CMDAT_EXP_BOOT_ACK) | (MSC_CMDAT_BOOT_MODE_A) | (MSC_CMDAT_DATA_EN));

	jz_mmc_start();

	ret = mmc_block_readp(SPL_SIZE, (u32 *)START_ADDR);
