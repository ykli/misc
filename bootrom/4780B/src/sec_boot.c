...
#define TCSM_SIGNATURE_OFFSET 0x7778
#define TCSM_SIGNATURE_SRC (START_ADDR + TCSM_SIGNATURE_OFFSET)
...
{
#if 0	/* 4780B rtc does not have HCR*/
...
__attribute__ ((__section__ (".start_text"))) void load_firmware()
{
...
__attribute__ ((__section__ (".start_text"))) int cache_init(void)
{
...
__attribute__ ((__section__ (".start_text"))) int init_se_mcu()
{
...
__attribute__ ((__section__ (".start_text"))) void move_data(void *src,int slen,void *target,int tlen)
{
...
__attribute__ ((__section__ (".start_text")))  void get_length_offset(unsigned int *off,unsigned int *len)
{
	unsigned int * dst = (unsigned int *)(TCSM_SIGNATURE_SRC);
	*off = *dst;
	*len = *(dst+1);
}
__attribute__ ((__section__ (".start_text")))  void read_efuse_protect()
{
...
int sec_boot(void)
{
	...
	ret = upload_data((void *)(START_ADDR + offset),length);
	if(ret != 0)
	{
		return -1;
	}
