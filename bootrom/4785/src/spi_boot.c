 ...
 *
 * Revision history:
 *  - 2012/03/21: Init version from jz4780 spi_boot. <Twxie>
...
do{							\
	REG_GPIO_PXPENC(0) = BIT18|BIT20|BIT21|BIT23;/* enable the pulling of SSI pins */\
	__ssi_disable(0);				\
}while (0)
...
do{							\
	__ssi_enable(0);				\
	REG_GPIO_PXPENS(0) = BIT18|BIT20|BIT21|BIT23;/* disable the pulling of SSI pins */\
}while (0)
...
int spi_boot(void)
{
	...
	if (length > SPL_SIZE) {
		...
		length = SPL_SIZE;			// force set 14K bytes
	}
	...
	/* check handle */
	addr = (unsigned char *)start_addr + jump_offset;
	length = (length - jump_offset) / 4;
	for (i = 0; i < length; i++) {
	...
	/* just jump to start_addr + jump_offset run. */
